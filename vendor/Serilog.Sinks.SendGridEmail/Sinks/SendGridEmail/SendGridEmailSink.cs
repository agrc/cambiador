...
namespace Serilog.Sinks.Email {
  public class SendGridEmailSink : PeriodicBatchingSink {
    ...
    private readonly ITextFormatter _subjectLineFormatter;
    ...
    public SendGridEmailSink(
        EmailConnectionInfo connectionInfo,
        int batchSizeLimit,
        TimeSpan period,
        ITextFormatter textFormatter,
        ITextFormatter subjectLineFormatter) : base(batchSizeLimit, period) {
    ...
    protected override async Task EmitBatchAsync(IEnumerable<LogEvent> events) {
      ...
      var status = await _client.SendEmailAsync(msg).ConfigureAwait(false);
    }
  }
}
